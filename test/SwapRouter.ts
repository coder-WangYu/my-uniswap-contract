...
describe("SwapRouter", async function () {
  ...
  it("exactInput", async function () {
    const { swapRouter, token0, token1, sender } = await deploySwapRouter();
    ...
    assert.equal(token1Amount, 97750848089103280585132n); // 大概是 97760 * 10 ** 18，按照 10000 的价格
  });
  ...
  it("exactOutput", async function () {
    ...
    assert.equal(
      1000000000000n * 10n ** 18n - token0Amount,
      1003011033103311n
    );
    // 检查收到的 tokenOut 数量
    const token1Amount = await token1.read.balanceOf([sender]);
  ...
  it("quoteExactInput", async function () {
    const { swapRouter, token0, token1 } = await deploySwapRouter();
    ...
    assert.equal(data.result, 97750848089103280585132n); // 10 个 token0 按照 10000 的价格大概可以换 97750 token1
  });
  ...
  it("quoteExactOutput", async function () {
    ...
    assert.equal(data.result, 1005019065211667067n); // 价格是 10000 大概需要 1 * 10n ** 18n token0，还有一些手续费
  });
});
