...
contract PositionManager is ERC721, IPositionManager {
    IPoolManager public poolManager; // 保存 PoolManager 合约地址

    /// @dev The ID of the next token that will be minted. Skips 0
    uint176 private _nextId = 1; // 用于生成唯一NFT ID的计数器，为每个流动性头寸生成唯一的NFT ID

    // 用一个 mapping 来存放所有 Position 的信息
    mapping(uint256 => PositionInfo) public positions;

    constructor(address _poolManger) ERC721("MyUniswapPosition", "MUSP") {
    ...
    {
        positionInfo = new PositionInfo[](_nextId - 1); // _nextId从1开始，数组长度为_nextId - 1
